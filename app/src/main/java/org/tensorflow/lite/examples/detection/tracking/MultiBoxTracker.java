...
import org.tensorflow.lite.examples.detection.DetectionEvent;
...
public class MultiBoxTracker {
  ...
  public MultiBoxTracker(final Context context) {
    ...
    boxPaint.setStrokeWidth(AppSharedPreferences.getLineWidth());
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      final String labelString =
          !TextUtils.isEmpty(recognition.title) && AppSharedPreferences.getShowTitle()
  ...
  private void processResults(final List<Recognition> results) {
    ...
    for (final Pair<Float, Recognition> potential : rectsToTrack) {
      ...
      boolean found = false;
      for(Disease disease: diseases){
        if(disease.getTitle().equals(potential.second.getTitle())){
      ...
      if(!found){
        ...
        currentColor++;
        Disease dis = new Disease(potential.second.getTitle(), color);
        dis.setCount(1);
        diseases.add(dis);
      }
    ...
    EventBus.getDefault().post(new DetectionEvent(diseases));
  }
